...
class Poly3(TrajectoryGenerator):
    def __init__(self, start_q, desired_q, T):
        ...
        Proszę zaimplementować wzory na a_0 do a_3 używając self.q_0 i self.q_k. Załóżmy, że prędkości na początku i na końcu wynoszą zero.
        """
        self.a_0 = self.q_0
        self.a_1 = 3*self.a_0 
        self.a_3 = self.q_k
    ...
    def generate(self, t):
        ...
        Zaimplementuj generator trajektorii dla swojego manipulatora.
        Trajektoria pozycyjna powinna być wielomianem trzeciego stopnia przechodzącym ze stanu początkowego q_0 do stanu pożądanego q_k.
        Pamiętaj, aby wyprowadzić także pierwszą i drugą pochodną.
        Skorzystaj z poniższego wzoru na wielomian z instrukcji.
        """
        t /= self.T
        q = self.a_3 * t**3 + self.a_2 * t**2 * (1 - t) + self.a_1 * t * (1 - t)**2 + self.a_0 * (1 - t)**3
        q_dot = 3*self.a_1*t**2 - 4*self.a_1*t + self.a_1 - 3*self.a_2*t**2 + 3*self.a_3*t**2 + 2*self.a_2*t - 3*self.a_0*t**2 + 6*self.a_0*t - 3*self.a_0
        q_ddot = 6*self.a_1*t - 4*self.a_1 - 6*self.a_2*t + 6*self.a_3*t + 2*self.a_2 - 6*self.a_0*t + 6*self.a_0
